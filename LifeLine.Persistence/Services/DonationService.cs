...
public class DonationService : IDonationService
{
    ...
    CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogError(
                    "Campaign {CampaignId} is verified but has no SubAccountCode — blocking donation.",
                    dto.CampaignId);
                return BaseResponse<InitiateDonationResponseDto>.Failure(
            ...
            var resolvedName = dto.IsAnonymous
                ? "Anonymous"
                : (donorName ?? dto.DonorName ?? "Anonymous");

            var totalAmount = dto.Amount;
            var platformFee = _paystack.PlatformFeeAmount; // e.g. ₦100
            var campaignAmount = totalAmount - platformFee;

            if (campaignAmount < 100)
                return BaseResponse<InitiateDonationResponseDto>.Failure(
                    $"Donation amount must be at least ₦{platformFee + 100} " +
                    $"(₦{platformFee} platform fee + minimum ₦100 to the campaign).",
                    statusCode: 400);
            ...
            // The campaign's subaccount receives the campaign share.
            // The platform fee naturally remains in the primary merchant account.
            var splits = new List<PaystackSplitEntryDto>
            {
                new() { SubaccountCode = campaign.SubAccountCode, ShareKobo = campaignAmountKobo }
            };
            ...
            if (!initResult.IsSuccess || initResult.Data is null)
                return BaseResponse<InitiateDonationResponseDto>.Failure(
                    ...
                    statusCode: initResult.StatusCode ?? 400);
